<div class="overflow-auto" style="display:none;background-color: rgba(0,0,0,0.5)" x-show="{{ $slug }}"
    :class="{ 'fixed inset-0 z-10 flex items-start justify-center': {{ $slug }} }">
    <div class="m-4 bg-gray-100 rounded-md shadow-2xl sm:m-8 w-96" x-show="{{ $slug }}"
        x-on:click.away="{{ $slug }} = false">
